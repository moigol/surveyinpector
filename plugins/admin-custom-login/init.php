...
function acl_weblizar_admin_custom_login_menu() {
	if(current_user_can('administrator')){
		$wl_admin_menu = add_menu_page( __( 'Admin Custom Login', WEBLIZAR_ACL ), __( 'Login Form', WEBLIZAR_ACL ), 'manage_options', 'admin_custom_login', 'acl_admin_custom_login_content', 'dashicons-art', 10 );
...
function acl_footer_func() {
	...
	jQuery(document).ready(function(){
		...
		jQuery('#registerform label[for="user_login"]').attr('id', 'log_input_lable');
		jQuery('#registerform label[for="user_email"]').attr('id', 'eml_input_labelmo');

		jQuery('p.message.register').append("<div class='icon-ph'><i class='fa fa-edit'></i></div>");
		...
		if (jQuery('#log_input_lable').length) {
			...
			jQuery("#pwd_input_lable").html("<?php echo $label_password; ?><div class='input-container'> <div class='icon-ph'><i class='fa <?php echo $password_input_icon; ?>'></i></div> <input id='user_pass' name='pwd' class='input' type='password' placeholder='<?php echo $pass_cust_lbl; ?>'></div>");
			
			jQuery('#eml_input_labelmo').html("<div class='input-container'> <div class='icon-ph'><i class='fa fa-envelope'></i></div> <input type='email' name='user_email' id='user_email' class='input' placeholder='Email'></div>");
