...
<div id="review" class="<?php echo esc_attr( $classes ); ?>">
	...
	<?php 
	// stars
	/* if ( $review['items'] && is_array( $review['items'] ) && empty( $review['disable_features'] ) ) : ?>
	...
	<div class="reviewed-item">
		<div class="row">
			...
			foreach ( $fields as $key => $data ) {
				...
				if ( 'url' === $data['name'] && ! isset( $data['part_of'] ) ) {
					if ( ! empty( $schema_data['url'] ) ) {
						...
						$more_link = '<a href="' . esc_url( $schema_data['url'] ) . '" class="reviewed-item-link" rel="nofollow noopener"><i class="fas fa-link"></i><span>' . esc_html( $more_text ) . '</span></a>';
						if ( ! empty( $schema_data['use_button_style'] ) ) {
							$url = '<ul class="review-links" style="padding-left: 0; padding-right: 0;"><li>' . $more_link . '</li></ul>';
						} else {
							$url = '<p>' . $more_link . '</p>';
						}
					}
			...
			if ( ! empty( $image ) ) {
				echo '<div class="reviewed-item-image col-md-5 col-sm-12">' . $image . '</div>'; // WPCS: xss ok.
			}
			if ( ! empty( $reviewed_item_data ) ) {
				echo '<div class="reviewed-item-data col-md-7 col-sm-12">'; 
				// review title
	...
	<?php if ( ! $is_embed && $review['user_review'] && ! $review['hide_visitors_rating'] ) : ?>
		<?php if ( ! wp_review_user_can_rate_features( $review['post_id'] ) ) : ?>
			<div class="user-review-area visitors-review-area col-lg-6 col-md-12 col-sm-12">
				<?php echo wp_review_user_rating( $review['post_id'] ); ?>
				<div class="user-total-wrapper">
					<h5 class="user-review-title"><?php esc_html_e( 'Quick Rate', 'wp-review' ); ?></h5>
	...
	<?php 
	// quick rate 
	 if ( ! $is_embed && ! empty( $review['enable_embed'] ) ) : ?>
