...
if ( 'theme' !== $comments_template && ( is_user_logged_in() || ! wp_review_option( 'registered_only' ) ) ) {
...
function wp_review_comments_template( $theme_template ) {
	if ( is_singular() ) {
		$review_type = wp_review_get_post_review_type( get_queried_object_id() );
		if ( ! $review_type ) {
...
/**
 * Filters comment template body class.
 *
...
if ( is_user_logged_in() || ! wp_review_option( 'registered_only' ) ) {
...
function wp_review_comment_fields_replace( $fields ) {
	...
	if ( isset( $fields['author'] ) ) {
		$fields['author'] = '<p class="wp-review-comment-form-author"><span class="icon-ph"><i class="fa fa-user"></i></span><input id="author" name="author" type="text" value="" size="30" placeholder="' . __( 'Name', 'wp-review' ) . '" /></p>';
	}
	if ( isset( $fields['email'] ) ) {
		$fields['email'] = '<p class="wp-review-comment-form-email"><span class="icon-ph"><i class="fa fa-envelope"></i></span><input id="email" name="email" type="text"  value="" size="30" placeholder="' . __( 'Email', 'wp-review' ) . '" /></p>';
	}
	if ( isset( $fields['url'] ) ) {
	...
	if ( isset( $fields['comment'] ) ) {
		...
		<p class="wp-review-comment-form-comment">
			<span class="icon-ph"><i class="fa fa-edit" aria-hidden="true"></i></span>
		...
		<?php
		if ( wp_review_show_comment_pros_cons( $post->ID ) ) :
			?>
...
function wp_review_comment_meta_box_fields( $comment ) {
	...
	<?php
	if ( wp_review_show_comment_pros_cons( $comment->comment_post_ID ) ) :
	...
	if ( $comment_qualifier ) {
		...
		<div class="wp-review-field">
			...
			<div class="wp-review-field-option">
				<select id="wp_review_comment_qualifier" name="wp_review_comment_qualifier">
					<option value=""><?php esc_html_e( 'Select', 'wp-review' ); ?></option>
					<option value="yes" <?php selected( $comment_qualifier, 'yes', true ); ?>><?php esc_html_e( 'Yes', 'wp-review' ); ?></option>
					<option value="no" <?php selected( $comment_qualifier, 'no', true ); ?>><?php esc_html_e( 'No', 'wp-review' ); ?></option>
				</select>
			</div>
		</div>
		<?php
...
/**
 * Saves our comment (from the admin area).
 *
...
function wp_review_comment_add_title_to_text( $text, $comment = null ) {
	...
	if ( is_admin() ) {
		...
		if ( WP_REVIEW_COMMENT_TYPE_COMMENT === $type ) {
			$rating = get_comment_meta( $comment_id, WP_REVIEW_COMMENT_RATING_METAKEY, true );
		} elseif ( WP_REVIEW_COMMENT_TYPE_VISITOR === $type ) {
			$rating = get_comment_meta( $comment_id, WP_REVIEW_VISITOR_RATING_METAKEY, true );
...
/**
 * Adds rating and title to the comment quick edit.
 *
...
/**
 * Adds extra comment data before processing.
 *
...
/**
 * Replaces "Comment" with "Review" in submit field.
 *
...
/**
 * Keeps "comment" class in 'wp_review_comment' comment type.
 *
...
/**
 * Enables avatar for 'wp_review_comment' comment type.
 *
...
/**
 * Updates user ratings total if comment status is changed.
 *
...
/**
 * Adds verified text to WooCommerce comment.
 *
