...
/**
 * Filters to add custom images sizes in Thumbnail selection box.
 *
 * @param array $sizes Image sizes.
 * @return array
 */
function wp_review_add_custom_image_size_names_choose( $sizes ) {
	return array_merge(
		$sizes,
		apply_filters(
...
add_filter( 'image_size_names_choose', 'wp_review_add_custom_image_size_names_choose' );
...
function wp_review_show_total( $echo = true, $class = 'review-total-only', $post_id = null, $args = array() ) {
	...
	switch ( $show_on_thumbnails_type ) {
		case 'author':
			...
			$total = wp_review_normalize_rating_value( $total, $type );

			if ( in_array( $type, array( 'point', 'percentage', 'thumbs' ), true ) ) {
		...
		case 'comments':
			$total = get_post_meta( $post_id, 'wp_review_comments_rating_value', true );
			$total = wp_review_normalize_rating_value( $total, $type );

			if ( 'point' === $user_type || 'percentage' === $user_type ) {
				$rating = sprintf( $wp_review_rating_types[ $user_type ]['value_text'], $total );
			} else {
				$rating = wp_review_user_comments_rating( $post_id );
			}
			break;
	}
...
function wp_review_get_user_rating_setup( $post_id ) {
	$default      = wp_review_option( 'global_user_rating', false ) ? WP_REVIEW_REVIEW_VISITOR_ONLY : wp_review_option( 'default_user_review_type', WP_REVIEW_REVIEW_DISABLED );
 ...
 *
 * @param int        $post_id Post ID.
 * @param WP_Comment $new     New comment.
 * @param WP_Comment $old     Old comment.
 *
...
function wp_review_locate_box_template( $template_name, $return_full_path = true ) {
	...
	foreach ( $paths as $path ) {
		if ( file_exists( $full_path = trailingslashit( $path ) . $template_name ) ) { // phpcs:ignore
			$located      = $full_path;
			$path_partial = $path;
			break;
		}
...
function wp_review_visitor_feature_rating( $post_id = null, $args = array() ) {
	...
	if ( isset( $GLOBALS['wp_review_features_rating_is_output'] ) ) {
		$is_output = $GLOBALS['wp_review_features_rating_is_output'];
	}
	...
	$output .= '<h5 class="user-review-title">' . __( 'Detailed Review', 'wp-review' ) . '</h5>';
	...
	if ( is_array( $features ) && ! empty( $features ) ) {
		foreach ( $features as $feature_id => $feature ) {
			...
			$value_text = ' - <span>' . sprintf( $rating_type['value_text'], wp_review_normalize_rating_value( $value, $type ) ) . '</span>';
			$output    .= '<li>';
			...
			if ( ! in_array( $type, array( 'star', 'thumbs', 'circle' ) ) && $user_has_reviewed ) {
...
function wp_review_user_comments_rating( $post_id = null, $args = array() ) {
	...
	if ( 'thumbs' === $type ) {
		return '<div class="review-thumbs">' . $value . '</div>';
	}
 ...
 *
 * @param bool $template Box template name. Default use current post box template.
 * @return array
 */
...
/**
 * Adds post type list table custom columns.
 */
	...
	<div class="updated notice-info wp-review-notice">
		<p>
			...
			printf(
				...
				__( 'Thank you for updating WP Review Pro. Your existing user ratings will show up after importing them in %s.', 'wp-review' ),
				'<a href="' . admin_url( 'options-general.php?page=wp-review-pro%2Fadmin%2Foptions.php#migrate' ) . '">' . __( 'Settings &gt; WP Review Pro &gt; Migrate Ratings', 'wp-review' ) . '</a>'
			);
			?>
		</p>
...
function wp_review_get_schema( $review ) {
	...
	if ( in_array( $review['schema'], apply_filters( 'wp_review_schema_force_nested_user_rating_types', array( 'SoftwareApplication', 'Recipe' ) ) ) && in_array( $review['rating_schema'], array( 'visitors', 'comments' ) ) ) {
		$nesting_mode = 'rating';
	}
	...
	if (
		...
		if ( $insert ) {
			if ( update_comment_meta( $insert, WP_REVIEW_VISITOR_RATING_METAKEY, $review ) ) {
				...
				$output['status'] = 'ok';
				if ( ! empty( $rating_data['features'] ) ) {
					$GLOBALS['wp_review_features_rating_is_output'] = true;

					$output['html'] = wp_review_get_review_box( $post_id );
					unset( $GLOBALS['wp_review_features_rating_is_output'] );
				} else {
...
/**
 * Switches to Network site, if global option is disabled in sub-site.
 *
...
/**
 * Gets option value from main-network site.
 *
...
/**
 * Filters capability settings group.
 *
 * @param string $cap Capability.
 * @return string
 */
function wp_review_filter_capability_settings_group( $cap ) {
	return 'wp_review_global_options';
}
add_filter( 'option_page_capability_wpreview-settings-group', 'wp_review_filter_capability_settings_group' );

/**
 * Registers capabilities group for MEmbers plugin.
 */
function wp_review_members_register_cap_group() {
...
add_action( 'members_register_cap_groups', 'wp_review_members_register_cap_group' );

/**
 * Registers capabilities for Members plugin.
 */
function wp_review_members_register_caps() {
...
add_action( 'members_register_caps', 'wp_review_members_register_caps' );

/**
 * Adds roles capabilities.
 */
function wp_review_add_roles_caps() {
...
add_action( 'admin_init', 'wp_review_add_roles_caps' );
...
/**
 * Removes <link> tags in HTML.
 *
 * @param string $html HTML.
 * @return string
 */
function wp_review_remove_link_tags( $html ) {
	if ( wp_review_is_amp_page() ) {
		$html = preg_replace( '/<link[^>]+?\/>/', '', $html );
	}
	return $html;
}
add_filter( 'wp_review_get_data', 'wp_review_remove_link_tags' );
